...
export default function AdminOrdersPage({ orders, filters }: Props) {
    ...
    return (
        <AdminLayout>
            ...
            {/* Bulk action bar */}
            {selected.size > 0 && (
                <div className="mb-3 flex items-center gap-3 rounded-lg bg-white border border-gray-200 px-4 py-2.5 text-gray-700 text-sm shadow-sm">
                    <span className="font-semibold text-[#e94560]">{selected.size} selected</span>
                    <span className="text-gray-300">·</span>
                    <span className="text-gray-500">Set status to</span>
                    <select
                        value={bulkStatus}
                        onChange={(e) => setBulkStatus(e.target.value)}
                        className="rounded-md bg-gray-50 border border-gray-200 px-2 py-1 text-xs text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#e94560]"
                    >
