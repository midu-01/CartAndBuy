...
type FormData = { name: string; description: string; price: string; sale_price: string; stock_qty: string; category_id: string; is_featured: boolean; is_active: boolean };
...
export default function AdminProductsPage({ products, categories, filters }: Props) {
    ...
    const form = useForm<FormData>({ name: '', description: '', price: '', sale_price: '', stock_qty: '', category_id: '', is_featured: false, is_active: true });

    function openCreate() { form.reset(); setEditing(null); setShowForm(true); }

    function openEdit(p: Product) {
        form.setData({ name: p.name, description: '', price: String(p.price), sale_price: String(p.sale_price ?? ''), stock_qty: String(p.stock_qty), category_id: String(p.category?.id ?? ''), is_featured: p.is_featured, is_active: p.is_active });
        setEditing(p); setShowForm(true);
    }

    function submitForm(e: React.FormEvent) {
        e.preventDefault();
        if (editing) { form.patch(`/admin/products/${editing.id}`, { onSuccess: () => setShowForm(false) }); }
